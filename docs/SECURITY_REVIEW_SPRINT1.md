...
1. **Before any deployment:** H-1 is fixed (fail closed); still remove the compose placeholder secret when production config is separated, then add rate limiting (H-4) on `/auth/*` and every AI-backed route.
